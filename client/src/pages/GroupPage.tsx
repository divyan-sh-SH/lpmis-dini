...
const createEmptyTransaction = (group_id?: string): TransactionCreate => ({
...
const createEmptyStock = (group_id?: string): StockCreate => ({
  stock_item: '',
  quantity: 0,
  group_id,
});

const createEmptyCart = (group_id?: string): CartItemCreate => ({
  item_name: '',
  store: '',
  cost: 0,
  note: '',
...
export default function GroupPage() {
  ...
  const [transactionForm, setTransactionForm] = useState<TransactionCreate>(createEmptyTransaction(groupId));
  const [stockForm, setStockForm] = useState<StockCreate>(createEmptyStock(groupId));
  const [cartForm, setCartForm] = useState<CartItemCreate>(createEmptyCart(groupId));
  ...
  useEffect(() => {
    ...
    setTransactionForm(createEmptyTransaction(groupId));
    setStockForm(createEmptyStock(groupId));
    setCartForm(createEmptyCart(groupId));
  ...
  async function handleAddTransaction() {
    if (!groupId) return;
    try {
      await createTransaction({ ...transactionForm, group_id: groupId });
  ...
  return (
    <div className="w-full">
      <header className="mb-5">
        <h1 className="text-4xl font-bold tracking-tight mb-2">Group Dashboard</h1>
        <p className="text-slate-500">Manage group transactions, stocks, and carts.</p>
      </header>
      ...
      <div className="space-y-8">
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-2xl font-bold">Group Transactions</h2>
              <p className="text-sm text-slate-500">View and manage transactions for this group.</p>
            </div>
            <button
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-2xl font-bold">Group Stocks</h2>
              <p className="text-sm text-slate-500">Track shared stock items and inventory.</p>
            </div>
            <button
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-2xl font-bold">Group Carts</h2>
              <p className="text-sm text-slate-500">Manage buying items for the group.</p>
            </div>
            <button
